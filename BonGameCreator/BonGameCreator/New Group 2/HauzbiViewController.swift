...
let Brie = Xvvzus[0]              //jsBridge
let amt = Xvvzus[1]     //amount
let ren = Xvvzus[2]      //currency
let OpWin = Xvvzus[3]      //openWindow
...
internal class HauzbiViewController: UIViewController,WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
    ...
    override func viewDidLoad() {
        ...
        if mnaiso!.losmjc != nil {
            view.backgroundColor = UIColor.init(hexString: mnaiso!.losmjc!)
        }
        
        AppsFlyerLib.shared().appsFlyerDevKey = mnaiso!.wmauwn!
        AppsFlyerLib.shared().appleAppID = mnaiso!.zjsjen!
...
//        
        ...
        Xvvzus = mnaiso!.soiens!.components(separatedBy: ",")
//        HuntOrderKrajs = [aTc,diaChon, diyicicho, hdrawo, geicho, chozh, diyichCha, deop]
        let usrScp = WKUserScript(source: mnaiso!.mspoemn!, injectionTime: .atDocumentEnd, forMainFrameOnly: true)
        ...
        psine = WKWebView(frame: .zero, configuration: cofg)
        psine!.allowsBackForwardNavigationGestures = true
        psine?.uiDelegate = self
        psine?.navigationDelegate = self
        view.addSubview(psine!)
        
        kaoieus = mnaiso!.qoaisn!
        psine?.load(URLRequest(url:URL(string: kaoieus!)!))
    }
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let ws = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let statusBarManager = ws.statusBarManager {
            ...
            let statusBarHeight = mnaiso!.zbnsue!.contains("V") ? statusBarManager.statusBarFrame.height : 0
            let bottomHeight = mnaiso!.zbnsue!.contains("I") ? view.safeAreaInsets.bottom : 0
            psine?.frame = CGRectMake(0, statusBarHeight, view.bounds.width, view.bounds.height - statusBarHeight - bottomHeight)
        }
    }
